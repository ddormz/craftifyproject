...
function avancesIDTable() {
  var proyecto_id_proyecto = window.location.pathname.split('/')[2];
  console.log(proyecto_id_proyecto);

  var avancesTbl = $(".tabla-avances-proyecto").DataTable({
    "responsive": true,
    "lengthChange": false,
    "autoWidth": false,
    "language": {
    ...
    columnDefs: [
      {
        ...
        orderable: false,
        render: function (data, type, row) {
          var buttons = '<a href="/editavanceproyecto/' + row.avance_id + '/" class="btn btn-warning btn-xs btn-flat"><i class="fas fa-pen"></i></a> ';
          buttons += '<a rel="remove" href="/elimavanceproyecto/' + row.avance_id + '/" class="btn btn-danger btn-xs btn-flat" id="botonEliminar"><i class="fas fa-trash-alt"></i></a>';
        return buttons;
        }
      }
    ],
...
document.addEventListener('DOMContentLoaded', avancesIDTable);